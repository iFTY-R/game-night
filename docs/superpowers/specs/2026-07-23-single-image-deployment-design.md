...
- 同时提供两种 Compose 模式：默认 `compose.yaml` 将应用进程放进一个 `app` 容器，`compose.multi.yaml` 额外将 `edge`、`api`、`realtime` 和 `worker` 拆成独立应用服务。
...
默认单服务模式由 `compose.yaml` 提供，只有一个应用服务 `app`，使用镜像内的 `serve-all` 入口在同一容器内启动上述四个进程。`serve-all` 是一个真正的进程主管，负责转发信号、回收子进程、在关键进程退出时结束容器，并为 API、realtime 和 worker 注入各自的数据库与 keyring 配置；不能用简单的 shell 后台命令替代。

两种模式共享同一个镜像、依赖服务和 `8080` 公网入口契约。单服务模式适合最少容器配置，多服务模式适合需要独立重启、资源限制和后续独立扩容的部署。两份编排应择一运行，不要求同时启动。
...
host:8080 -> container:8080
              ...
              +-- app
                    ...
                    +-- worker    no listener
              |
              +-- PostgreSQL / Redis / MinIO are separate Compose services
```

单服务模式的内部监听地址必须绑定回环地址，避免同一容器内的 API、realtime 或 owner RPC 被意外当成额外部署入口。`app` 容器重启时四个应用进程整体重启。

### 4.2 额外多服务模式（`compose.multi.yaml`）
...
### 8.1 默认单服务应用服务（`compose.yaml`）

默认单服务编排提供一个 `app` 服务，使用同一个镜像的 `serve-all` 命令：
...
`app` 内部由 edge 监听 `:8080`，API 监听 `127.0.0.1:8081`，realtime 监听 `127.0.0.1:8090` 和 `127.0.0.1:8091`；只有 edge 的 `8080` 映射到宿主机。单服务模式的进程主管把三个进程专用 DSN 映射为各自的 `GAME_NIGHT_DATABASE_URL`，不能让多个进程共享错误的数据库角色。

### 8.2 额外多服务应用服务（`compose.multi.yaml`）
...
应用通过服务名访问这些依赖；生产环境可以将同名环境变量替换为外部地址。应用镜像不依赖 Compose 服务名才能启动，但默认示例应提供完整可运行的本地编排。

当前 Compose 的 PostgreSQL、Redis 和 MinIO 都使用私有网络上的明文连接，因此默认环境为 `development`。将 `GAME_NIGHT_ENVIRONMENT` 切换为 `production` 前，必须分别配置 PostgreSQL TLS、Redis TLS 和 HTTPS 对象存储 endpoint；应用配置会拒绝生产环境的明文 PostgreSQL、Redis 和对象存储连接。

### 8.4 Migration service

migration 使用同一镜像和同一组数据库 secret：
...
`migrate` 不应与长期应用服务形成无条件的长期依赖关系。生产发布系统必须显式等待 migration 成功，再启动多服务模式的 `api`、`realtime`、`worker`、`edge`，或单服务模式的 `app`。
...
数据库、Redis、对象存储、keyring、内部 token 和 checkpoint sink 继续使用现有进程配置。多服务模式直接给每个容器设置 `GAME_NIGHT_DATABASE_URL`；单服务模式通过 `GAME_NIGHT_API_DATABASE_URL`、`GAME_NIGHT_REALTIME_DATABASE_URL` 和 `GAME_NIGHT_WORKER_DATABASE_URL` 保持角色分离，再由 `serve-all` 写入各自子进程环境。API 使用全量 keyring staging volume，worker 只挂载 PII、TOTP 和 audit 三类最小 keyring；staging 文件以运行 UID 所有、`0400` 权限提供。secret 只能通过部署环境或 secret file 注入，不能进入前端 bundle、Docker layer、Compose 仓库文件或日志。
...
- 两种模式都使用同一个应用镜像，不引入第二套构建产物；PostgreSQL、Redis 和 MinIO 仍是独立依赖服务。

默认由部署者按运维需求选择一种模式。单机最简部署可使用单服务模式；需要进程级隔离或独立扩容时使用多服务模式，并保持 edge 路由契约不变。
...
4. 已落地默认 `compose.yaml` 单服务编排，以及额外 `compose.multi.yaml` 多服务编排、数据库角色初始化、Redis、MinIO 和 migration profile。
...
- 默认 `compose.yaml` 只创建一个长期应用服务 `app`；`compose.multi.yaml` 额外提供 `edge`、`api`、`realtime` 和 `worker` 四个长期应用服务。
...
- 默认 `compose.yaml` 使用 `serve-all` 在一个 `app` 容器中管理 edge、API、realtime 和 worker；`compose.multi.yaml` 额外把四个进程拆为独立服务。
- 采用一个 Go edge 作为唯一公网入口，不把 Nginx 作为镜像运行时依赖。
- 两种模式都只发布 `8080`；其他端口只存在于容器回环地址或 Compose 私有网络。
- 前端静态文件、ConnectRPC 和 WebSocket 使用同源入口。
- PostgreSQL、Redis、S3/MinIO 由 Compose 或外部基础设施提供。
