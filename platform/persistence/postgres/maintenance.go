...
type ExpiryCleanup struct{ queries *sqlcgen.Queries }

// NewExpiryCleanup binds the worker-only maintenance port to a pool.
func NewExpiryCleanup(pool *pgxpool.Pool) *ExpiryCleanup {
	if pool == nil {
		return nil
	}
	return &ExpiryCleanup{queries: sqlcgen.New(pool)}
}
...
func (cleanup *ExpiryCleanup) RunReport(ctx context.Context) (CleanupReport, error) {
	...
	if err := json.Unmarshal(payload, &report); err != nil || !report.valid() {
		return CleanupReport{}, errInvalidCleanupReport
	}
	return report, nil
}
...
func (report CleanupReport) valid() bool {
	return report.ExpiredResults >= 0 && report.DeletedResults >= 0 && report.ExpiredChallenges >= 0 &&
